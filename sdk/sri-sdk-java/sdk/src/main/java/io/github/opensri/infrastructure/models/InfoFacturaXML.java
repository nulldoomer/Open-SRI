...
public class InfoFacturaXML {
    ...
    public static InfoFacturaXML fromDomain(Invoice invoice, IssuerProfile profile){
        ...
        xml.contribuyenteEspecial = profile.specialTaxPayer() == null
                ? ""
                : profile.specialTaxPayer().number();
        xml.obligadoContabilidad = profile.accountingObligation().name();

        xml.tipoIdentificacionComprador = invoice.client().identification().
                identificationType().getCode();
        ...
        xml.totalConImpuestos = invoice.totals().totalTaxes().stream()
                .map(TotalImpuestoXML::fromDomain)
                .toList();

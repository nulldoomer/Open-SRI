...
class SRISOAPGatewayIntegrationTest {
  ...
  void setUp() throws Exception {
    ...
    SendReceiptProxy receiptProxy =
        new SendReceiptProxy(WsdlLocations.SRI_PRUEBAS_RECEPCION_COMPROBANTES_WSDL);
    AuthorizationProxy authorizationProxy =
        new AuthorizationProxy(WsdlLocations.SRI_PRUEBAS_AUTORIZACION_COMPROBANTES_WSDL);
  ...
  void should_connect_to_sri_and_receive_response() {
    // 1. Serializar
    String xml =
  ...
  void should_connect_to_authorization_service() {
    // Intentamos autorizar la clave de prueba (obviamente no estará autorizada)
    AuthorizationResponse response = gateway.sendAuthorization(accessKey);

    assertNotNull(response);
    System.out.println("Integración SRI - Estado Autorización: " + response.status());

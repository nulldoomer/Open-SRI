...
public class DetalleXML {
    ...
    public static DetalleXML fromDomain(InvoiceItem invoiceItem) {
        ...
        xml.detallesAdicionales = invoiceItem.additionalDetails().stream()
                .map(DetalleAdicionalXML::fromDomain)
                .toList();

        //TODO: Añadir comentario explicativo
        xml.impuestos = invoiceItem.taxes().stream()
                .map(ImpuestoXML::fromDomain)
                .toList();

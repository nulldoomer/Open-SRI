 ...
 *
 * <p>This infrastructure component is responsible for turning the intermediate
 * {@link FacturaXML} representation into the final XML payload expected by the SRI.
 * It also collaborates with {@link AccessKeyGenerator} so the generated document can
 * include environment-dependent identification data when the serialization flow is completed.
 *
...
class InvoiceXmlSerializer implements XmlSerializer<FacturaXML> {
     ...
     *
     * <p>The serializer is expected to transform the JAXB-compatible invoice structure
     * into the final XML string required by the SRI invoice schema.
     *

...
public record ClientIdentification(IdentificationType type, String number){

    public ClientIdentification {
        Objects.requireNonNull(type, "Identification type cannot be null");
        Objects.requireNonNull(number, "Identification number cannot be null");
    }

    private void validateRuc(String number){
        new Ruc(number);
    }

    private void validateNationalId(String number){
        if (number.length() != 10) {
